...
| src/core/combat.ts | 战斗核心：attackSide（部位判定，防御姿态参数化）、calcEvade（**R4-6 双轴回避**：速/运×对应轴系数+地形闪避，经 statValue 入修正管线）、calcStrike/calcBattleForecast（**R4-2 统一公式**：伤害段权重基数×克制（cap 3.0）×背刺（总封顶 4.0）−防御−地形防，先乘后减 max 内 floor；命中接对应轴回避）、resolveBattle；反击与攻击候选含普攻；calcAoeForecast/resolveAoeBattle、applyDamageToUnit | §4.2~§4.5/§4.7/§4.9 | tests/core/combat.test.ts |
...
| src/core/status.ts | 状态系统：九种 ActiveStatus（chant 锁定格/stealth/buff 衰减/stance/charge 蓄力/dot 灼烧/…）、resolveArmor、tickStatuses、interruptChant、applyBuff、refreshAuras、statValue（模板+buff+姿态+冲锋/狂战被动；R4-6 AttrKey 扩 spd/lck 轴——回避入修正管线） | §4.10/§4.12/§4.9 | tests/core/status.test.ts |
...
| src/config/combat.ts | 战斗数值：DAMAGE_ARMOR_MATRIX（§4.2 定稿物理梯度，R4-3 法术行移出）、PART_BONUS、COMBAT_PARAMS（命中/**双轴回避系数 evadeCoeffs**〔R4-6：phys/mag×spd/lck 四值独立，默认速 0 运 3，R4-8 定稿〕/追击/递增距离惩罚 base15+step10/克制 cap 3.0/总封顶 4.0）、RANGE_PARAMS（R4-5 属性条件射程阈值）、EFFECT_PARAMS | §4.2~§4.4/§4.7 | —（tests/core/combat 间接） |
