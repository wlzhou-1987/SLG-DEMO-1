...
| src/render/prep-board.ts | PrepBoard 类：战前画布——部署区高亮 + **R16-4 标识叠加（drawMarkers deploy 半透明于绿高亮之上）** + 我方站位渲染 + 点击调整（选中/移动/交换/区外 onInvalid），复用 Camera/HexRenderer/InputHandler（R19 render 同构 applyView/resetView 世界系包装） | §7.0 | tests/render/prep-board.test.ts |
