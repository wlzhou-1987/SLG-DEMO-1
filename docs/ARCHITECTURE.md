...
| src/config/combat.ts | 战斗数值：DAMAGE_ARMOR_MATRIX（§4.2 定稿物理梯度，R4-3 法术行移出）、PART_BONUS、COMBAT_PARAMS（命中/追击/递增距离惩罚 base15+step10/克制 cap 3.0/总封顶 4.0）、RANGE_PARAMS（R4-5 属性条件射程阈值）、EFFECT_PARAMS | §4.2~§4.4/§4.7 | —（tests/core/combat 间接） |
