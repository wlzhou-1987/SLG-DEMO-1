...
| src/render/sprite-cache.ts | 棋子贴图资源管理器（R15-4）：SpriteCache 按 URL 懒加载+缓存（loader 可注入、node 可测），未就绪/失败恒 null（失败=缺失同回落）+ spriteCache 单例；clear 供测试/HMR | §5.3/§7.5 | tests/render/sprite-cache.test.ts |
