...
- 进度与需求以 `docs/BACKLOG.md` 为唯一真源（含历史里程碑）；代码结构与文件职责以 `docs/ARCHITECTURE.md` 为真源
...
- **进度单一真源**：项目进度（含历史里程碑 M1~M6）只在 `docs/BACKLOG.md` 记录；README、GAME-DESIGN §10 的里程碑为冻结快照，禁止回写。完成/未完成标注必须与代码事实一致，只有当轮验证证据（§6）支持时才可标记"已完成"
...
历史里程碑（M1~M6 已全部完成，2026-09-04）见 `docs/BACKLOG.md` §0；后续需求与进度亦以 `docs/BACKLOG.md` 为唯一真源。
